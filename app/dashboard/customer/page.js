...
const Page = () => {
  ...
  useEffect(() => {
    axios
      .get(Endpoint.BASE_URL + Endpoint.CUSTOMER, {
        headers: {
          ...
          {customers &&
            customers.map((a) => (
              ...
              >
                ...
                <td>{a.name}</td>
                <td>{a.email}</td>
                <td>{a.detail && a.detail.phone}</td>
                <td>
                  {a.detail &&
                    `${a.detail.province}, ${a.detail.city}, ${a.detail.detail_address}`}
                </td>
                <td>
                  {a.detail && a.detail.photo && (
                    <img
                      className="rounded w-28"
                      src={`${Endpoint.CUSTOMERIMAGE}${a.detail.photo}`}
                    />
                  )}
                </td>
              </tr>
